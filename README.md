...
Этот проект демонстрирует реализацию простой сверточной нейронной сети (CNN) на языке C для обучения на наборе данных CIFAR-10. Реализация включает:
- Сверточный слой с валидной свёрткой,
- Активацию ReLU,
- Max pooling,
- Полносвязный (fully-connected, FC) слой,
- Функцию softmax с кросс-энтропийной потерей,
- Сохранение и загрузку весов модели,
...
Проект реализует базовую сверточную нейронную сеть, способную обрабатывать цветные изображения размером 32×32 (CIFAR) и классифицировать их по 10 классам. Основные возможности:
- Обучение на наборе данных CIFAR-10
